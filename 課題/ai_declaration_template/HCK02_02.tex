...
・マイクから入力された音信号の波形をprocessing側に送信する方法\par
・processing側の処理内容の理解とコードの案だし\par
...
マイク音信号の受信方法や，マイクからの入力波形と楽曲波形を視覚的に比較・描画する方法について案出しを指示した．提案されたコードに対し，
実機による検証および別の生成AIによるダブルチェックを行い，必要に応じて修正を加えたため一部修正して使ったを選択した．
...
Arduino日本語リファレンスや公式ドキュメントなどで確認をしハルシネーションが発生していないか確認した．更に，
...
出力内容が高度化されたり，逆に間違った解釈をされてしまったりと一概に出力結果を鵜呑みにしてはいけないことがわかった．
新しいチャットに同様のプロンプトを投げることや，別の生成AIを用いて検証を行うことが
ハルシネーションを鵜呑みにしない方法であると考えた．

\section*{6. 自分で工夫した点}
マイクから入力された音信号の波形(赤)とprocessingで作成した楽曲の波形(緑)を同時にウィンドウに描画することで，
正常に課題の要件が満たせているか可視化を行った．また，Arduino側でdelayMicrosecondsを挿入することにより，シリアル通信の同期を安定させ，音信号の
欠落や遅延を抑制する工夫を施した．
