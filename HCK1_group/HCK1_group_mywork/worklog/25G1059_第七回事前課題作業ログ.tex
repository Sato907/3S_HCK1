...
\begin{itemize}
  ...
  \item 主要成果：
    \begin{itemize}
      ...
      \item {指揮デバイスのBPM・音量変化機構のプログラム作成（No.2113）}
    \end{itemize}
  \item 重大課題（影響度：高）：
    \begin{itemize}
...
50\% &
可変抵抗器の接続不良 &
可変抵抗器の接続不良により計画比で遅延．筐体は段ボールで制作する予定． \\
\hline
...
40\% &
可変抵抗器の接続不良 &
可変抵抗器のBPM5段階写像するプログラムを実装したが，機構のハードウェア固定は未完で計画比やや遅延．未検証． \\
\hline
...
\subsection{日ごとの作業ログ}
    ...
    進行中 &
    80\% &
    無表示の根本原因（framebufferの翻訳単位不一致）を特定し，matrix操作をDisplay.cppに集約してBPM表示を確定させた（通信部は未完）． \\
    \hline
    ...
    進行中 &
    60\% &
    CdS検証を手による遮光方式に変更し，A0値による明暗判定を実機で確認した． \\
    \hline
    ...
    進行中 &
    50\% &
    ADXL345による振り動作検知を実装．可変抵抗器をブレッドボードに上手く接続できず停滞．筐体は段ボールで制作する予定． \\
    \hline
