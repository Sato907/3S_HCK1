...
\begin{itemize}
  ...
  \item 重大課題（影響度：高）：
    \begin{itemize}
      ...
      指揮デバイスの製図および本体・センサの実装がほとんど進捗しなかった．}
    \end{itemize}
\end{itemize}
...
40\% &
可変抵抗器の接続不良 &
可変抵抗器の接続が安定せず，回路構成が確定しないため進捗が停滞． \\
\hline
...
20\% &
可変抵抗器の接続不良 &
可変抵抗器をブレッドボード上に上手く接続できず，実装がほとんど進まなかった． \\
\hline
...
\midrule
可変抵抗器がブレッドボード上で安定接続できない & スライド式可変抵抗器の端子形状がブレッドボードに適合しないため． & 高 & 高 & スライド式を継続使用する場合は基盤にはんだ付けする．しぼり（回転式）の可変抵抗器に変更する場合は部品を貰う． & 6/9 & 対応中 \\
\midrule
