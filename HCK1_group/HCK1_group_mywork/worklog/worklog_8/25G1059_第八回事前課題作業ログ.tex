...
95\% &
通信部の結合待ち &
6/10の授業にて音量の実装フェーズの達成を確認．残りはUDP送信（通信部）の結合のみ． \\
\hline
...
\begin{itemize}
  ...
  \item 目標：指揮デバイスの最終固定およびテストフェーズへの移行
    \begin{itemize}
      ...
      \item テストフェーズに移行し，以下の検証を実施する（図\ref{fig:gant8}参照）．
        \begin{itemize}
          \item 音量の再調整確認（No.3111）
          \item 音量機能の検証（No.3112）
          \item 複数人での音量変化の確認（No.3121）
          \item 音量変化と楽器の音の同期確認（No.3122）
          \item LEDマトリクスの音の確認（No.3131）
          \item 楽曲情報の検証（No.3141）
          \item 楽曲観覧データの確認（No.3151）
          \item 開始信号の受信確認（No.3211）
          \item 開始信号データの確認（No.3212）
          \item 合奏の音のバランスの確認（No.3221）
          \item LEDマトリクスの反映確認（No.3241）
          \item 観覧車によるBPM変更確認（No.3242）
        \end{itemize}
    \end{itemize}
