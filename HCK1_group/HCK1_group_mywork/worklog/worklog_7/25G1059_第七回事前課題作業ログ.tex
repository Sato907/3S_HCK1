...
70\% &
筐体上部は回路完成待ち &
段ボールで筐体の箱部分を制作．ボタン動作は検証済．可変抵抗器の基盤設置（はんだ付け）はメンバーに委任中で，動作確認・加速度センサの検証・筐体上部の設置は6/10に実施予定． \\
\hline
...
80\% &
無 &
可変抵抗器の動作確認．可変抵抗器から読み取った値を5段階にして出力するプログラム（通信部以外）は実装・検証済．残りはUDP送信（通信部）と機構のハードウェア固定． \\
\hline
...
\subsection{日ごとの作業ログ}
    ...
    6/4 & 2113 & 可変抵抗器の動作確認 & 0.5h &
    進行中 &
    進行中 &
    50\% &
    slider\_testでスライド式可変抵抗器のアナログ値が正しく読み取れること（破損していないこと）を確認．(なお，別の可変抵抗器で検証をしているため，基盤設置後に再度検証が必要．) \\
    \hline
    ...
    進行中 &
    80\% &
    音量用可変抵抗器を接続，可変抵抗器から読み取った値を5段階にして出力するプログラム（通信部以外）の実装・検証を完了． \\
    \hline
...
次に，現在のGCを図\ref{fig:gant5}（実装フェーズ）および図\ref{fig:gant6}（テストフェーズ）に示す．達成した項目は緑色で塗られている．

\begin{figure}[htbp] % h:ここ, t:上, b:下, p:独立ページ
    \centering % 中央揃え
    \includegraphics[width=0.9\columnwidth]{gant5.png} % 横幅を列幅の90%に指定
    \caption{現在のガントチャート（実装フェーズ）} % 図の説明
    \label{fig:gant5} % 本文中で参照するためのラベル
\end{figure}

\begin{figure}[htbp] % h:ここ, t:上, b:下, p:独立ページ
    \centering % 中央揃え
    \includegraphics[width=0.9\columnwidth]{gant6.png} % 横幅を列幅の90%に指定
    \caption{現在のガントチャート（テストフェーズ）} % 図の説明
    \label{fig:gant6} % 本文中で参照するためのラベル
\end{figure}
...
\midrule
可変抵抗器がブレッドボード上で安定接続できない（No.2112/2113に影響） & スライド式可変抵抗器の端子形状がブレッドボードに適合しないため． & 高 & 高 & スライド式可変抵抗器を基盤に設置する方針とした（はんだ付けはメンバーに委任中）． & 6/10 & 対応中 \\
\midrule
LEDマトリクスが無表示になる & matrix操作が複数箇所に分散していたため． & 中 & 中 & 描画処理をDisplay.cppに集約し，表示を確定させた． & 5/30 & 対応済 \\
\midrule
基盤を設置したスライド式可変抵抗器が動作するかが未検証（No.2112/2113に影響） & 小指を骨折したため，はんだ付けが行えずメンバーに委任し手元にないため． & 高 & 高 & 水曜の授業で受け取り後，速やかに動作確認及び回路の実装を行う． & 6/10 & 対応中 \\
\bottomrule
\end{tabularx}
...
\begin{itemize}
  \item {}指揮デバイスの筐体は，計画書および第六回で想定した3Dプリンタ製（Tinkercad設計）から，現段階では段ボール製に変更している．
\end{itemize}
...
\begin{itemize}
  \item 添付資料：
    \begin{itemize}
      ...
      \item No.2113(可変抵抗器の動作確認)のプログラム:\url{https://github.com/Sato907/3S_HCK1/tree/main/HCK1_group/HCK1_group_mywork/slider_test}
