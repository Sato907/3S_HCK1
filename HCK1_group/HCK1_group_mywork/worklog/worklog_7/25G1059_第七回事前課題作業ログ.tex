...
\begin{itemize}
  ...
  \item 重大課題（影響度：高）：
    \begin{itemize}
      ...
      本体・センサの実装（No.2112）は筐体上部の設置が回路完成待ちのため一部残っている．}
    \end{itemize}
\end{itemize}
...
\subsection{日ごとの作業ログ}
    ...
    進行中 &
    40\% &
    shiki\_deviceを新規作成し，可変抵抗器2本の移動平均→5段階化→BPM/音量送信を実装した． \\
    \hline
    ...
    完了 &
    100\% &
    第七回作業ログを現在のガントチャートに整合させ，図を修正した． \\
    \hline

    6/4 & 2112 & 本体・センサの実装 & 2h &
    進行中 &
    進行中 &
    58\% &
    スライド式可変抵抗器を基盤に設置し，ブレッドボードでの接続不良を解消した． \\
    \hline
    ...
    進行中 &
    60\% &
    BPM用可変抵抗器(A0)の生値デバッグ出力を追加し，読み取り値を5段階にして出力する処理をシリアルモニタで確認した． \\
    \hline
    ...
    進行中 &
    80\% &
    音量用可変抵抗器をA5へ接続変更し，可変抵抗器から読み取った値を5段階にして出力するプログラム（通信部以外）の実装・検証を完了した． \\
    \hline
    ...
    進行中 &
    69\% &
    ボタン(D3)の検証用デバッグ出力を追加し，押下による演奏開始/終了のトグル動作を確認した． \\
    \hline
